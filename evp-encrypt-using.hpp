...
//    }
    ...
    void gen_params(byte key[KEY_SIZE], byte iv[BLOCK_SIZE])
    {
        memcpy(key, "012345678901234567890123456789ab", KEY_SIZE);
        memcpy(iv, "0123456789abcdef", BLOCK_SIZE);
...
//            throw std::runtime_error("RAND_bytes for iv failed");
    ...
    void aes_encrypt(const byte key[KEY_SIZE], const byte iv[BLOCK_SIZE], const secure_string& ptext, secure_string& ctext)
    {
        ...
        int rc = EVP_EncryptInit_ex(ctx.get(), EVP_aes_256_cfb128(), NULL, key, iv);
        if (rc != 1)
    ...
    {
        EVP_CIPHER_CTX_free_ptr ctx(EVP_CIPHER_CTX_new(), ::EVP_CIPHER_CTX_free);
        int rc = EVP_DecryptInit_ex(ctx.get(), EVP_aes_256_cfb128(), NULL, key, iv);
        if (rc != 1)
...
//        return 0;
    ...
    void decrypt(
                 //                 const std::string key, const std::string iv,
                 const secure_string &ctext, secure_string &ptext) {
...
//        std::cout << ptext.length() << std::endl;
//        auto p = ptext.data();
//        
